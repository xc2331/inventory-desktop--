  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-stone-100">
      <header className="flex items-center gap-3 border-b border-stone-200 bg-white px-6 py-4">
        ...
        <h1 className="flex-1 text-lg font-semibold text-stone-800">{t('cat_title')}</h1>
        <button
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 overflow-y-auto p-6">
        {adding && (
          <form onSubmit={handleAdd} className="mb-4 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
              <label className="block">
                <span className="mb-1 block text-xs font-medium text-stone-500">{t('cat_icon')}</span>
                <input
                  type="text"
                  value={newCat.icon}
              ...
              <label className="block">
                <span className="mb-1 block text-xs font-medium text-stone-500">{t('cat_key')}</span>
                <input
                  type="text"
                  value={newCat.key}
              ...
              <label className="block">
                <span className="mb-1 block text-xs font-medium text-stone-500">{t('cat_name')}</span>
                <input
                  type="text"
                  value={newCat.name}
              ...
              <label className="block">
                <span className="mb-1 block text-xs font-medium text-stone-500">{t('cat_name_en')}</span>
                <input
                  type="text"
        ...
        <div className="space-y-2">
          {categories.map((cat) => {
            ...
            if (editingId === cat.id) {
              return (
                <div key={cat.id} className="rounded-xl border border-emerald-300 bg-white p-4 shadow-sm">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                    <label className="block">
                      <span className="mb-1 block text-xs font-medium text-stone-500">{t('cat_icon')}</span>
                      <input
            ...
            return (
              <div
                key={cat.id}
                className="group flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm transition hover:shadow-md"
              >
                <span className="text-xl">{cat.icon || '🏷️'}</span>
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-medium text-stone-800">{categoryDisplayName(cat, lang)}</div>
                  <div className="text-xs text-stone-400">{cat.key}</div>
                </div>
