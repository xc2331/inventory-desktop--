...
创建物品时若 `itemNo` 留空（不传或传空字符串），应用会自动生成格式为 `WP-YYYYMMDD-NNN` 的编号，例如 `WP-20260730-001`。其中 `NNN` 是当天创建序号。Agent 无需手动生成编号，除非用户明确指定了自定义编号。

### 2. 分类别名自动归一化

创建 / 更新物品时传入的 `category` 会被自动归一化：
...
应用启动时也会自动合并 `tool` / `tools` 等重复分类，保证数据一致。Agent 传入中文别名也能正常工作，但建议优先使用规范 key（见分类对照表）以减少歧义。
...
- HTTP 服务仅绑定 `127.0.0.1`，不会暴露到局域网或公网。
