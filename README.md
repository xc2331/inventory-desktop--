...
- **v1.3.15**：Toast 点击定位（UX-09）：新增/编辑物品后的 Toast 点击可滚动到对应卡片并高亮 1.4s，非物品类 Toast 行为不变；Toast 上显示 `cursor-pointer` 提示
...
- **v1.3.12**：批量操作面板拖拽排序（UX-06）：顶部拖拽排序预览条，最多显示 8 张迷你卡片；松开即按新顺序排列；支持 `bulk_dragReorder` / `bulk_order` 中英双语
- **v1.3.11**：表单校验即时反馈（UX-05）：核心字段 blur 触发校验 + 错误清除 + 错误提示弹性动效 + 字段抖动动画 + 错误边框高亮
...
- **v1.3.4**：过期徽章英文 i18n 修复；`?` 快捷键排除输入框聚焦状态；搜索无结果空状态优化新增「清除筛选」按钮
- **v1.3.3**：过期遮罩重设计（柔和渐变 + 半透明）；Toast 通知队列化（最多堆叠 3 条）
- **v1.3.2**：批量设置数量 Bug 修复（`set` 模式更新所有选中项）；幽灵列白名单（移除 `purchase_amount`/`barcode`/`photo`）；暗色模式切换白闪修复；`MotionConfig` 全局配置
- **v1.3.1**：修复电子材料库白屏（`SearchBar` 组件导入缺失）；全局异步错误监听；材料库、位置图、平面图编辑器 ErrorBoundary 兜底；标签计数胶囊样式；Toast 底部进度条；TagModal 实时搜索；过期日期日历选择器
- **v1.3.0.a**：修复材料库卡片大小/视图模式/卡片风格重启后丢失；电子材料库接口从 `/api/materials` 改为 `/api/e-materials`，响应体标题加【电子材料】前缀；新增 `/api/e-materials/types` 接口
...
- **v1.2.14**：Agent 接口创建/更新物品位置与分类同步修复；位置解析支持多种分隔符并自动去空格；写入与同步在同一事务；分类显示名优先保留中文
...
- **v1.2.10**：平面图编辑器点击子区域时删除绑定位置的防御；bindLocationId 查找增加空值防御；已删除位置显示提示
- **v1.2.9**：位置地图及平面图编辑器白屏修复；错误边界、数据加载空值防御；右侧抽屉动画优化；平面图编辑器 i18n 补全
