...
- **v1.3.4**：过期徽章 `{n} 天后过期` 接入中英双语 i18n 模板替换；`?` 快捷键打开快捷键面板时排除 `input`/`textarea` 聚焦状态，避免搜索框内误触；搜索无结果时新增「清除筛选」按钮，方便一键回退
- **v1.3.3**：过期卡片警示重设计，从红色全屏遮罩 + 无限抖动图标改为顶部红色渐变警示带 + 边框 + 徽章（不遮挡卡片内容，抖动动画只播 2 次后静止）；Toast 从单槽覆盖改为队列化（最多堆叠 3 条，前一条被顶掉的问题修复）
- **v1.3.2**：修复批量修改数量 `set` 模式只更新首条的问题，现在会正确更新所有选中项；批量修改字段白名单移除 `purchase_amount`/`barcode`/`photo` 幽灵列，避免批量操作时报 SQL 错；暗色主题启动白闪修复（`html/body` 直设背景色 + 系统偏好媒体查询兜底）；全局 `MotionConfig` 包裹所有动画，为后续「关闭动画」设置接管做准备
- **v1.3.1**：
...
- **v1.3.3**：过期遮罩重设计（柔和渐变 + 半透明）；Toast 通知队列化（避免多条通知重叠）
- **v1.3.2**：批量设置数量 Bug 修复；幽灵列白名单；暗色模式切换白闪修复；MotionConfig 全局配置
- **v1.3.1**：
